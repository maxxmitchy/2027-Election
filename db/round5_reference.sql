...
DROP TABLE IF EXISTS ai_answer_dependency, ai_answer_state, ai_answer, result, analysis, calculation, dependency_edge, observation_version, observation, dataset_version, dataset, methodology_version, evidence, claim, retrieval_event, source, election_result, candidacy, election, person, office_holding, office, geography, comparison, record_version CASCADE;

CREATE TABLE record_version (version_id text PRIMARY KEY, entity_id text NOT NULL, entity_type text NOT NULL, version_number integer NOT NULL CHECK (version_number >= 1), transaction_from timestamptz NOT NULL, status text NOT NULL CHECK (status IN ('current','superseded','stale','invalid')), change_type text NOT NULL, previous_version_id text NULL REFERENCES record_version(version_id), UNIQUE(entity_id, version_number));
CREATE OR REPLACE FUNCTION enforce_version_chain() RETURNS trigger LANGUAGE plpgsql AS $$ DECLARE p record; BEGIN IF NEW.version_number=1 AND NEW.previous_version_id IS NOT NULL THEN RAISE EXCEPTION 'v1 cannot have predecessor'; END IF; IF NEW.version_number>1 THEN IF NEW.previous_version_id IS NULL THEN RAISE EXCEPTION 'later version requires predecessor'; END IF; SELECT entity_id,version_number INTO p FROM record_version WHERE version_id=NEW.previous_version_id; IF NOT FOUND OR p.entity_id<>NEW.entity_id OR p.version_number<>NEW.version_number-1 THEN RAISE EXCEPTION 'predecessor must be immediate predecessor of same entity'; END IF; END IF; IF EXISTS(SELECT 1 FROM record_version WHERE entity_id=NEW.entity_id AND transaction_from=NEW.transaction_from) THEN RAISE EXCEPTION 'transaction timestamp must be unique within entity'; END IF; RETURN NEW; END $$;
...
CREATE TABLE election(election_id text PRIMARY KEY, office_id text NOT NULL REFERENCES office(office_id));
...
CREATE OR REPLACE FUNCTION allowed_status_transition(old_status text,new_status text) RETURNS boolean LANGUAGE sql IMMUTABLE AS $$ SELECT (old_status,new_status) IN (('potential','declared'),('declared','nominated'),('nominated','registered'),('registered','withdrawn'),('registered','disqualified'),('registered','elected'),('registered','potential')) $$;
CREATE OR REPLACE FUNCTION candidacy_transition_guard() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.status<>OLD.status AND NOT allowed_status_transition(OLD.status,NEW.status) THEN RAISE EXCEPTION 'invalid candidacy status transition'; END IF; RETURN NEW; END $$;
...
CREATE TABLE methodology_version(methodology_version_id text PRIMARY KEY,methodology_id text NOT NULL,version_number integer NOT NULL,previous_methodology_version_id text REFERENCES methodology_version(methodology_version_id),UNIQUE(methodology_id,version_number));
...
CREATE INDEX dependency_upstream_idx ON dependency_edge(upstream_ref); CREATE INDEX dependency_downstream_idx ON dependency_edge(downstream_ref); ALTER TABLE dependency_edge ADD CONSTRAINT no_self_dependency CHECK(upstream_ref<>downstream_ref);
...
CREATE OR REPLACE FUNCTION reject_dependency_cycle() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF EXISTS(WITH RECURSIVE walk(v) AS(SELECT NEW.downstream_ref UNION SELECT d.downstream_ref FROM dependency_edge d JOIN walk w ON d.upstream_ref=w.v) SELECT 1 FROM walk WHERE v=NEW.upstream_ref) THEN RAISE EXCEPTION 'dependency cycle detected'; END IF; RETURN NEW; END $$;
...
-- Immutable answer content/lineage; mutable lifecycle state is separate.
CREATE TABLE ai_answer(answer_id text PRIMARY KEY,answer_text text NOT NULL,generated_at timestamptz NOT NULL,database_snapshot_ref text NOT NULL,as_of timestamptz,version integer NOT NULL DEFAULT 1);
CREATE TABLE ai_answer_state(answer_id text PRIMARY KEY REFERENCES ai_answer(answer_id),status text NOT NULL CHECK(status IN('draft','published','stale','superseded')));
...
CREATE OR REPLACE FUNCTION deny_answer_mutation() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'AI answer content and lineage are immutable'; END $$;
CREATE TRIGGER answer_immutable BEFORE UPDATE OR DELETE ON ai_answer FOR EACH ROW EXECUTE FUNCTION deny_answer_mutation();
CREATE OR REPLACE FUNCTION deny_answer_dependency_mutation() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'AI answer dependencies are immutable'; END $$;
CREATE TRIGGER answer_dependency_immutable BEFORE UPDATE OR DELETE ON ai_answer_dependency FOR EACH ROW EXECUTE FUNCTION deny_answer_dependency_mutation();

CREATE VIEW version_bitemporal AS SELECT v.*,lead(transaction_from) OVER(PARTITION BY entity_id ORDER BY version_number) AS transaction_to FROM record_version v;
CREATE OR REPLACE FUNCTION dependent_versions(seed text) RETURNS TABLE(version_id text) LANGUAGE sql AS $$ WITH RECURSIVE walk(v) AS(SELECT seed UNION SELECT d.downstream_ref FROM dependency_edge d JOIN walk w ON d.upstream_ref=w.v WHERE d.status='active') SELECT v FROM walk WHERE v<>seed $$;
