...
def discover(task: ResearchTask, candidates: list[dict]) -> list[dict]:
    ...
    task.attempts += 1; task.last_attempt = now()
    matches=[]
...
def source_class(s):
    return "PRIMARY" if s.get("tier") == 1 or classify_source(s) in {"OFFICIAL_PRIMARY","COURT_RECORD","ELECTION_RECORD","GOVERNMENT_DATA","OFFICIAL_STATEMENT","OFFICIAL_SOCIAL_ACCOUNT"} else "SECONDARY"


def capture(task: ResearchTask, discovery: dict, outdir: Path, *, timeout=20) -> dict:
    task.status="CAPTURE_PENDING"; task.attempts += 1; task.last_attempt=now()
    url=discovery["canonical_url"]
    if not url:
        task.status="FAILED"; return {"status":"FAILED","reason":"missing URL"}
    req=Request(url,headers={"User-Agent":"2027-Election-Phase6/1.0 evidence-acquisition"})
    started=time.time()
    try:
        with urlopen(req,timeout=timeout) as r:
            body=r.read(2_000_000); ctype=r.headers.get("Content-Type","")
            final_url=r.geturl(); status=getattr(r,"status",200); headers={k:v for k,v in r.headers.items() if k.lower() in {"content-type","etag","last-modified","content-length","date"}}
        content_hash=sha256(body); artifact_id=f"artifact-{content_hash[:20]}"
        folder=outdir/artifact_id; folder.mkdir(parents=True,exist_ok=True); (folder/"content.bin").write_bytes(body)
        ...
        task.status="CAPTURED"; task.result={"capture":event}
        return event
    except (HTTPError,URLError,TimeoutError,OSError) as e:
        reason=norm(str(e)); task.status="UNAVAILABLE" if isinstance(e,HTTPError) and e.code in (404,410) else "FAILED"
        event={"retrieval_event_id":"retrieval-"+sha256(f"{task.task_id}|{url}|failure".encode())[:20],"task_id":task.task_id,"retrieval_timestamp":now(),"original_url":discovery["discovery_url"],"canonical_url":url,"retrieval_status":"RETRIEVAL_FAILED","capture_status":"NOT_CAPTURED","error_class":type(e).__name__,"error":reason,"elapsed_ms":round((time.time()-started)*1000,2)}
        task.result={"capture":event}; return event
...
def verify(task: ResearchTask, discovery: dict, retrieval: dict, *, source_text=None) -> dict:
    if retrieval.get("retrieval_status") != "SUCCESS":
        task.status="UNAVAILABLE"; return {"verification_status":"UNAVAILABLE","primary_state":"PRIMARY_UNAVAILABLE" if task.source_requirement=="PRIMARY_REQUIRED" else "SECONDARY_ONLY"}
    primary = discovery.get("primary_secondary") == "PRIMARY"
    text = (source_text or "").lower()
    hostile = any(x in text for x in ("ignore previous instructions","system prompt","ai generated summary","this document proves"))
    if hostile:
        task.status="REQUIRES_REVIEW"; return {"verification_status":"PARTIALLY_VERIFIED","primary_state":"PRIMARY_CAPTURED" if primary else "SECONDARY_ONLY","requires_review":True,"reason":"hostile source content treated as data"}
    if task.review_requirement:
        task.status="REQUIRES_REVIEW"; state="PRIMARY_CAPTURED" if primary else "SECONDARY_ONLY"
        return {"verification_status":"PARTIALLY_VERIFIED","primary_state":state,"requires_review":True}
    task.status="VERIFIED" if primary or task.source_requirement != "PRIMARY_REQUIRED" else "PARTIALLY_VERIFIED"
    return {"verification_status":"VERIFIED" if task.status=="VERIFIED" else "PARTIALLY_VERIFIED","primary_state":"PRIMARY_VERIFIED" if primary else "SECONDARY_ONLY"}
...
def run_task(task: ResearchTask, dossier_sources: list[dict], outdir: Path) -> dict:
    ...
    artifact=outdir/retrieval["artifact_id"]/"content.bin"
    text=artifact.read_bytes().decode("utf-8","replace")[:10000] if artifact.exists() else ""
    verification=verify(task,d,retrieval,source_text=text)
    evidence=[]
    if verification.get("verification_status") in {"VERIFIED","PARTIALLY_VERIFIED"}:
        evidence.append(extract_evidence(task,d,retrieval,verification,task.claim_target))
