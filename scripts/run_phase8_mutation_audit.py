...
def audit():
 ...
 case('M6_remove_gap',lambda d,r:d[c].update(research_gaps=[]),lambda d,r:'research_gaps' in d[c] and d[c]['research_gaps']==[])
 case('M7_force_gap_closed',lambda d,r:d[c]['research_gaps'].append({'status':'RESOLVED','gap_id':'forced'}),lambda d,r:any(g.get('status')=='RESOLVED' for g in d[c].get('research_gaps',[])))
 ...
 case('M26_publish_with_blocking_gap',lambda d,r:d[c]['research_gaps'].append({'status':'OPEN'}),lambda d,r:publication_readiness(d[c],r)['publication_decision'] in {'QUALIFIED_WITH_LIMITATIONS','NEEDS_MORE_EVIDENCE','BLOCKED'})
