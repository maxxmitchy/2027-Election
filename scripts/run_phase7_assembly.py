...
from phase7_dossier import CANDIDATES,BLOCKED,METHODOLOGY_VERSION,build_dossier,assemble_investigation_records,dossier_diff,snapshot,quality_gate
...
def main():
 ...
 if removed:
  ...
  v1["research_gap_ids"]=sorted(set(v1["research_gap_ids"]+["gap-v1-"+removed])); v1["quality_gate"]=quality_gate(v1); v1["status"]=v1["quality_gate"]["recommended_state"]
 ...
 write("phase-7-coverage.json",{"investigations":len(invs),"tasks":sum(len(i["research_tasks"]) for i in invs),"dossier_versions":4,"v1_v2_added_evidence":diff["ADDED_EVIDENCE"],"historical_rebuilds":2,"candidates":list(CANDIDATES),"candidate_4":BLOCKED,"reviews_queued":len(queue),"open_gaps":sum(len(d["research_gaps"]) for d in ds.values()),"methodology_version":METHODOLOGY_VERSION}); md("phase-7-coverage.md","Phase 7 — Coverage",json.loads((REPORT/"phase-7-coverage.json").read_text()))
 write("phase-7-snapshots.json",snaps); write("phase-7-v1-v2.json",{"v1":v1,"v2":v2}); write("phase-7-historical-rebuild.json",historical)
 print("PHASE7_ASSEMBLY_OK")
