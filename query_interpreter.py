...
def _scope(q: str):
    ...
    if "three candidates" in ql or ("tinubu" in ql and "obi" in ql and "atiku" in ql): return list(CANDIDATE_ALIASES)
    return found
...
def _domain_entity(ql):
    ...
    if "vote" in ql or "ballot" in ql: return "election","presidential_vote_count","Nigeria"
...
def interpret(raw_question: str) -> dict:
    ...
    if operation=="COMPARISON" and len(scope)<2:
        ambiguities.append("Comparison requires at least two compatible subjects."); status="PARTIALLY_INTERPRETED"
    if len(scope)>1 and operation not in {"COMPARISON","COUNT"}:
        status="PARTIALLY_INTERPRETED"; ambiguities.append("Multiple candidate subjects require an explicit comparison or aggregate operation.")
...
def interpret_and_validate(raw_question):
    ...
    q["validation"]={"deterministic_retrieval_only":True,"llm_dependency":False,"raw_question_is_evidence":False,"security_instructions_ignored":bool(SECURITY_RE.search(raw_question or ""))}
    return q
