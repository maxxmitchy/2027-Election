"""Deterministic Phase 7 research-investigation and dossier assembly layer."""
...
def controlled_investigations(as_of="2026-08-30"):
    s=[("bola-ahmed-tinubu","economic-record","Economic record investigation","What evidence documents the economic record during Tinubu's presidency?"),("bola-ahmed-tinubu","election-history","Election-history investigation","What documentary evidence establishes Tinubu's election history?"),("peter-gregory-obi","anambra-fiscal","Anambra fiscal investigation","What evidence documents Peter Obi's Anambra fiscal record?"),("peter-gregory-obi","party-office","Party/office chronology investigation","What evidence establishes Peter Obi's party and office chronology?"),("atiku-abubakar","adc-legal","ADC legal-status investigation","What evidence establishes the ADC legal status relevant to Atiku Abubakar?"),("atiku-abubakar","federal-policy-ncp","Federal policy/NCP investigation","What evidence documents Atiku's role in the National Council on Privatisation and federal policy?")]
    return [make_investigation(*x[:2],x[3],as_of=as_of) for x in s]
...
def claim_type(c):
    t=str(c.get("claim_type","")).upper(); return {"FACT":"DOCUMENTED_ACTION","CALCULATED_FACT":"QUANTITATIVE_RESULT","CAUSAL":"CAUSAL_PROPOSITION","STATEMENT_OCCURRENCE":"PUBLIC_STATEMENT","ASSESSMENT":"CONTESTED_CLAIM"}.get(t,t if t else "UNKNOWN")
def source_class(s): return "PRIMARY" if s.get("tier")==1 else "SECONDARY"
def source_index(d): return {s.get("id"):s for s in d.get("sources",[]) if s.get("id")}
...
def build_dossier(root,cid,*,version=1,as_of="2026-08-30",database_snapshot="runtime-reference"):
    d=load_candidate(root,cid); si=source_index(d); sr=[]
    for s in d.get("sources",[]):
        if temporal_ok(s,as_of): sr.append({"source_id":s.get("id"),"source_class":source_class(s),"source_type":s.get("type"),"url":s.get("url"),"retrieval_date":s.get("retrieval_date"),"reliability":s.get("reliability"),"source_version_id":f"sv-{s.get('id')}","provenance_complete":bool(s.get("id") and s.get("url"))})
    sids={s["source_id"] for s in sr}; er=[]
    for e in d.get("evidence",[]):
        if e.get("source_id") in sids: er.append({"evidence_id":e.get("id"),"source_id":e.get("source_id"),"source_version_id":f"sv-{e.get('source_id')}","evidence_relationship":e.get("relationship"),"claim_ids":e.get("claim_ids",[]),"status":e.get("status","UNASSESSED"),"provenance_complete":bool(e.get("id") and e.get("source_id") and si.get(e.get("source_id"),{}).get("url"))})
    eids={e["evidence_id"] for e in er}; cr=[]
    for c in d.get("claims",[]):
        st=str(c.get("status","UNKNOWN")).upper(); state={"VERIFIED":"SUPPORTED","QUALIFIED":"PARTIALLY_SUPPORTED","INSUFFICIENT_EVIDENCE":"INSUFFICIENT_EVIDENCE","DISPUTED":"DISPUTED"}.get(st,"UNVERIFIED")
        cr.append({"claim_id":c.get("id"),"candidate_id":cid,"claim_text":c.get("claim"),"claim_type":claim_type(c),"status":state,"evidence_ids":[x for x in c.get("evidence_ids",[]) if x in eids],"source_ids":[e["source_id"] for e in er if c.get("id") in e.get("claim_ids",[])],"investigation_ids":[],"review_ids":[],"confidence_qualification":c.get("causal_classification"),"as_of":as_of,"methodology_version":METHODOLOGY_VERSION,"provenance":{"source":"pilot-record","source_claim_id":c.get("id")}})
    gaps=build_gaps(d,cid); reviews=[]
    for c in cr:
        if c["claim_type"] in {"CAUSAL_PROPOSITION","CONTESTED_CLAIM","LEGAL_STATUS","QUANTITATIVE_RESULT"} or c["status"] in {"DISPUTED","INSUFFICIENT_EVIDENCE"}: reviews.append({"review_id":f"review-{c['claim_id']}","claim_id":c["claim_id"],"reason":"material uncertainty or high-impact interpretation","required_reviewer_type":"domain_reviewer","status":"QUEUED","evidence_dependencies":c["evidence_ids"],"created_at":now(),"methodology_version":METHODOLOGY_VERSION})
    inv=[i for i in controlled_investigations(as_of) if i["candidate_id"]==cid]; invids=[i["investigation_id"] for i in inv]
    for c in cr:c["investigation_ids"]=invids
    out={"dossier_id":f"dossier-{cid}","candidate_id":cid,"version_number":version,"created_at":now(),"as_of":as_of,"methodology_version":METHODOLOGY_VERSION,"source_versions":[x["source_version_id"] for x in sr],"evidence_ids":[x["evidence_id"] for x in er],"claim_ids":[x["claim_id"] for x in cr],"investigation_ids":invids,"research_gap_ids":[g["gap_id"] for g in gaps],"review_ids":[r["review_id"] for r in reviews],"identity":{"person":d.get("person"),"candidacy":d.get("candidacies",[])},"party_history":d.get("party_memberships",[]),"office_history":d.get("officeholdings",[]),"election_history":d.get("election_results",[]),"public_statements":[c for c in cr if c["claim_type"]=="PUBLIC_STATEMENT"],"related_public_conversation":[c for c in cr if c["claim_type"]=="PUBLIC_STATEMENT],"documented_actions":[],"policies":[],"economic_record":d.get("observations",[]),"legal_record":d.get("legal_events",[]),"contested_claims":[c for c in cr if c["status"] in {"DISPUTED","INSUFFICIENT_EVIDENCE"}],"corrections":d.get("corrections",[]),"uncertainty":[c for c in cr if c["status"]!="SUPPORTED"],"research_gaps":gaps,"reviews":reviews,"sources":sr,"evidence":er,"claims":cr,"investigations":inv,"database_snapshot":database_snapshot,"generation_metadata":{"generator":"phase7_dossier_assembly","candidate_source":"pilot-record"}}
    out["quality_gate"]=quality_gate(out); out["status"]=out["quality_gate"]["recommended_state"]
    out["content_hash"]=digest({k:v for k,v in out.items() if k!="content_hash"}); return out
...
def quality_gate(d):
    ...
    failures=[k for k,v in checks.items() if not v]; state="BLOCKED" if failures else ("IN_REVIEW" if d.get("research_gaps") or any(r["status"]!="APPROVED" for r in d.get("reviews",[])) else "QUALIFIED")
