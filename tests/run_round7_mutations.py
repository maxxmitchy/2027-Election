...
'M4':('break predecessor validation','tests/test_round7.py::test_predecessor_validation',lambda s:s.replace('p.version_number<>NEW.version_number-1','p.version_number<>NEW.version_number',1)),
...
def mutate_schema_dir(src,dst):
    ...
    candidates=[]
    def has_ref(x):
        if isinstance(x,dict): return ('$ref' in x) or any(has_ref(v) for v in x.values())
        if isinstance(x,list): return any(has_ref(v) for v in x)
        return False
    for p in dst.glob('*.json'):
        try:
            if has_ref(json.loads(p.read_text())): candidates.append(p)
        except Exception: pass
    if not candidates: raise RuntimeError('M8 could not find a schema containing $ref')
    p=candidates[0]; obj=json.loads(p.read_text())
    changed=False
    def change(x):
        nonlocal changed
        if isinstance(x,dict):
            for k,v in list(x.items()):
                if k=='$ref' and isinstance(v,str) and not v.startswith(('http:','https:')) and not changed:
                    x[k]='missing-round7.schema.json'; changed=True
                else: change(v)
        elif isinstance(x,list):
            for v in x: change(v)
    change(obj); p.write_text(json.dumps(obj,indent=2))
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='round7-mut-') as td:
        ...
        for mid,(defect,testid,_) in MUTATIONS.items():
            mb=td/f'{mid}-base.sql'; me=td/f'{mid}-ext.sql'; mb.write_text(mutate_file(BASE.read_text(),mid)); me.write_text(mutate_file(EXT.read_text(),mid)); sd=td/f'{mid}-schemas'; mutate_schema_dir(SCHEMA_DIR,sd) if mid=='M8' else shutil.copytree(SCHEMA_DIR,sd)
            r=run_test(testid,mb,me,sd); sensitive=r.returncode!=0
    ...
    print(json.dumps(results,indent=2)); raise SystemExit(0 if all(x.get('sensitivity')=='YES' for x in results if 'mutation_id' in x) else 1)
