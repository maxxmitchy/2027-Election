...
def validate_answer(a):
    ...
    p=a.get("provenance",{})
    required=("answer_id","query_id","interpretation_version","methodology_version","database_snapshot","generation_timestamp")
    if any(not p.get(k) for k in required): return False
    iq=a.get("interpreted_query",{})
    if any(cid not in {"bola-ahmed-tinubu","peter-gregory-obi","atiku-abubakar"} for cid in iq.get("candidate_scope",[])): return False
    if a.get("review_information",{}).get("status")=="NOT_A_SOURCE" and a.get("review_information",{}).get("reviewed") is True: return False
    ...
    if iq.get("operation")=="PUBLIC_CONVERSATION":
        for row in a.get("related_public_conversation",[]):
            if row.get("semantic_rule")!="statement_occurrence_is_not_independent_truth": return False
    return True

def run():
    seeds={
      "M1_remove_source_provenance":"How many votes did Tinubu get in 2023?",
      "M2_remove_evidence_relationship":"What did Atiku say about ADC?",
      "M3_unknown_to_false":"Does the database establish that Peter Obi left Anambra with zero debt?",
      "M4_insufficient_to_positive":"Did Tinubu cause inflation to rise?",
      "M5_remove_contradiction":"What conflicting evidence exists about Anambra's debt during Obi's tenure?",
      "M6_remove_correction_history":"What changed in the evidence concerning ADC's legal status during Atiku's 2026 candidacy?",
      "M7_remove_calculation_inputs":"How did Nigeria's headline inflation change during the selected Tinubu period?",
      "M8_alter_calculation_result":"How did Nigeria's headline inflation change during the selected Tinubu period?",
      "M9_change_candidate_id":"How many votes did Tinubu get in 2023?",
      "M10_remove_as_of":"As of 2026-05-01, what party was Peter Obi recorded as belonging to?",
      "M11_current_for_historical":"As of 2026-05-01, what party was Peter Obi recorded as belonging to?",
      "M12_statement_as_truth":"What did Atiku say about ADC?",
      "M13_review_as_evidence":"What did Atiku say about ADC?",
      "M14_remove_methodology":"How many votes did Tinubu get in 2023?",
      "M15_change_geography":"How did Nigeria's headline inflation change during the selected Tinubu period?",
      "M16_change_metric_unit":"How did Nigeria's headline inflation change during the selected Tinubu period?",
      "M17_remove_unavailable_source_state":"What did Atiku say about ADC?",
      "M18_drop_limitations":"Did Tinubu cause inflation to rise?",
    }
