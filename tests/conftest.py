import json, os, pathlib, subprocess, datetime, pytest
...
def pytest_runtest_makereport(item, call):
    ...
    results.append({'test_id':item.nodeid,'expected':'test completes without an assertion/error','actual':'PASS' if rep.passed else (str(rep.longrepr)[:2000] if rep.failed else rep.outcome),'status':'PASS' if rep.passed else ('FAIL' if rep.failed else 'PARTIAL'),'severity':'critical' if rep.failed else 'info'})
    item.config._round7_results=results
