...
def validate(m, expected):
    ...
    # The source presentation must preserve every phase-4 source classification and URL.
...
def run():
    ...
    # Exact coverage recomputation is the oracle; this is a presentation-integrity test, not a second factual engine.
    ...
    mutations={
      ...
      "M5_unknown_to_false":lambda m:m.update({"answer_status":"ANSWERED","answer_text":"FALSE"}),
      ...
      "M12_temporal_to_causal":lambda m:m["answer_text"]="Tinubu caused the economic outcomes.",
      ...
      "M15_implemented_to_success":lambda m:m["coverage"]["candidates"]["tinubu"] if False else m["coverage"]["candidates"]["bola-ahmed-tinubu"]["domains"][7].update({"coverage":"HIGH","status":"DOCUMENTED_OUTCOME"}),
      "M16_remove_legal_event":lambda m:m["coverage"]["candidates"]["atiku-abubakar"]["domains"][9].update({"coverage":"HIGH"}),
      "M17_allegation_to_finding":lambda m:m["answer_text"]="The allegation is established as fact.",
      ...
      "M25_alter_calculation_output":lambda m:m.update({"answer_text":"Coverage calculation result: 999"}),
