...
def make_question(question: str, *, candidate_scope: tuple[str,...] | None = None, as_of: str | None = None) -> ResearchQuestion:
    ...
    domain = {"QUANTITATIVE":"economic","CAUSAL":"economic","POLICY":"policy","LEGAL":"legal","ELECTORAL":"electoral","PUBLIC_CONVERSATION":"public_conversation","COMPARATIVE":"comparative"}.get(qtype,"general")
...
def decompose(rq: ResearchQuestion) -> list[dict]:
    ...
    items = [
        ("scope", "What exactly is within the question's candidate, time and geographic scope?"),
        ("facts", "Which factual propositions must be established?"),
        ("evidence", "What evidence is required for each material claim?"),
        ("primary", "Which claims require primary-source verification?"),
        ("gaps", "What evidence is missing, unavailable or not yet investigated?"),
    ]
    ...
    if rq.question_type in {"CAUSAL","COMPOSITE"}:
        items += [("causal", "What mechanism, competing explanations and causal evidence would be required?"), ("limitations", "What prevents temporal association from being treated as causation?")]
    if rq.question_type in {"POLICY","COMPOSITE"}:
        items += [("policy", "What was proposed, announced, legislated, approved, implemented, reversed or associated with a documented outcome?")]
    if rq.question_type in {"LEGAL","COMPOSITE"}:
        items += [("legal", "What is the procedural chronology and current legal status?")]
    if rq.question_type in {"ELECTORAL","COMPOSITE"}:
        items += [("election", "What are the distinct nomination, candidacy, ballot, result, declaration and litigation states?")]
    if rq.question_type in {"PUBLIC_CONVERSATION","COMPOSITE"}:
        items += [("statement", "What artifact establishes that a person made the statement, and what evidence independently tests the proposition itself?")]
...
def dossier_summary(root: Path, cid: str) -> dict:
    d=load_candidate(root,cid)
    sources=d.get("sources",[]); claims=d.get("claims",[]); obs=d.get("observations",[]); calcs=d.get("calculations",[])
    primary=[s for s in sources if source_class(s)=="PRIMARY"]
    gaps=[]
...
def investigate(root: Path, question: str, *, candidate_scope: tuple[str,...] | None=None, as_of: str | None=None) -> dict:
    started=time.perf_counter(); rq=make_question(question,candidate_scope=candidate_scope,as_of=as_of); plan=decompose(rq); reqs=evidence_requirements(rq,plan)
    candidates={cid:dossier_summary(root,cid) for cid in rq.candidate_scope}
    gaps=[g for c in candidates.values() for g in c["research_gaps"]]
    sources=[{"candidate_id":cid,**{k:v for k,v in s.items() if k in {"id","tier","type","url","retrieval_date","reliability"}},"source_class":source_class(s),"verification_state":"RETRIEVED"} for cid,c in candidates.items() for s in c["sources"]]
    review_needed=any(r["required_review"] for r in reqs)
    answerability="PARTIALLY_ANSWERABLE" if gaps or review_needed else "ANSWERABLE"
    status="READY_FOR_REVIEW" if review_needed else answerability
...
def task_queue(investigation: dict) -> list[dict]:
    ...
    for r in investigation["investigation"]["evidence_requirements"]:
        typ="VERIFY_PRIMARY_SOURCE" if r["preferred_primary_source"] else "RETRIEVE_SOURCE"
        tasks.append({"task_id":f"task-{r['requirement_id']}","task_type":typ,"investigation_id":investigation["investigation"]["question"]["question_id"],"claim":r["target_claim"],"evidence_requirement":r["requirement_id"],"priority":0,"blocking":r["required_review"] or r["preferred_primary_source"],"provenance":investigation["provenance"],"state":"OPEN","completion_evidence":None})
    for g in investigation["research_gaps"]:
        tasks.append({"task_id":f"task-{g['gap_id']}","task_type":"RESOLVE_RESEARCH_GAP","investigation_id":investigation["investigation"]["question"]["question_id"],"claim":g["claim"],"evidence_requirement":None,"priority":0,"blocking":g["blocking"],"provenance":investigation["provenance"],"state":"OPEN","completion_evidence":None})
